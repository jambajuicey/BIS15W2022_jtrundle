...
```r
ggplot(data=iris, mapping=aes(x=Species, y=Petal.Length)) #geom not specified
```
...
ggplot(data = homerange, mapping = aes(x = log10.mass, y = log10.hra)) +
  geom_jitter() #makes plots with over-plotting a bit cleaner, doesn't work too great here though
```
...
In the R Console we can see that RStudio removed a bunch of rows (the ones with NAs) and didn't include them in the plot. 
...
Geom_bar is just for counts, geom_col specifies both an x and y axis. 


```r
homerange %>% 
  filter(family=="salmonidae") %>% 
  select(common.name, log10.mass) %>% #this is optional, you get the same result
...
homerange %>% 
  filter(family=="salmonidae") %>% 
  ggplot(aes(x=common.name, y=log10.mass))+
  geom_bar(stat="identity") #geom_col is better, is the exact same thing
```
...
## 2 herbivore       158
```
There are more herbivores in mammals. 

3. Make a bar plot that shows the masses of the top 10 smallest mammals.

```r
homerange_mammals_small <- homerange_mammals %>% 
  select(common.name, log10.mass) %>% 
  arrange(log10.mass) %>% 
  top_n(-10, log10.mass)
homerange_mammals_small
```
