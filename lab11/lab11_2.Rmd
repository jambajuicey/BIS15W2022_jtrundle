...
```{r}
display.brewer.pal(9,"BrBG") #number is how many different colors there are, code after comma is pallete code from website
```
```{r}
...
```{r}
p+scale_fill_brewer(palette = "Paired")+ #picks colors that go together, can replace "paried" with color code, but will only include as many colors as there are variables
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
```{r}
my_palette <- paletteer_d("khroma::sunset")
my_palette
```
...
```{r}
p+scale_fill_manual(values=my_palette)+ #using colors from object we named as "my_pallete"
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
homerange %>% 
  ...
  geom_point() +
  xlim(0, 4) + #focus in on a certain part of data/
  ylim(1, 6)
```
