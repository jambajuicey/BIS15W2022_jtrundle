...
```{r}
deserts %>% 
  filter(species_id=="DM"|species_id=="DL") %>% 
  group_by(year, species_id) %>% 
  summarise(n=n())
```

```{r}
deserts$year <- as.factor(deserts$year) #changing year to factor so it doesn't count is as a value, but more as a catgory
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "black", fill = "darkseagreen4")+ #bins are like number of bars in histogram (R can do it itself, but you can also change it), also alpha does the transparency
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="deepskyblue4", alpha  =0.4, color = "black")+ #density does smooth line
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(alpha=0.7, color = "black", fill = "lavenderblush1")+
  labs(title = "Homerange Histogram",
       x = "Log 10 Homerange")+
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_density(alpha=0.7, color = "black", fill = "darkblue")+
  labs(title = "Homerange Histogram",
       x = "Log 10 Homerange")+
  theme(plot.title = element_text(size=rel(1.5), hjust = 0.5))
```
...
homerange %>% 
  ggplot(aes(x=log10.hra))+
  geom_histogram(aes(y = ..density..), alpha=0.7, color = "black", fill = "lavenderblush1")+ #have to add the aes(y = ..density..) to overlay the line on the histogram
...
#in cases where you want to change continuous variables to be categorical
...
```{r}
glimpse(homerange) #mass_catergory has been added to the end of the dataset
```
...
homerange %>% 
  ...
  labs(title = "Taxonomic Classes in Homerange Categories",
       x = "Range Category",
       y = "Count")
```
