...
```r
religion <- readr::read_csv("data/relig_income.csv")
```
...
These data are untidy because observations are grouped by religion across a single row. Also the income variables are spread out over multiple column names when they should be one. 

"Income" is a variable, but they are values in the above data that should be all in one column. 
...
The days and values in the day columns should each have separate columns, but they do not in this data. 
