...
expenditures <- 
  USPersonalExpenditure %>% 
  as_tibble() %>% #tibble makes data frame
...
heartrate3 <- heartrate2 %>% 
  ...
  pivot_longer(-c(patient, sex),
               names_to = "drug",
               values_to = "heartrate") %>% 
  arrange(drug, patient) %>% 
  select(drug, patient, sex, heartrate)
heartrate3
```

```
## # A tibble: 24 × 4
##    drug  patient  sex   heartrate
##    <chr> <chr>    <chr>     <dbl>
##  1 a     Frank    m            84
##  2 a     Hawkeye  m            64
##  3 a     Henry    m            88
...
```r
heartrate3 %>% 
  unite(patient_sex, sep = "_", patient, sex)
```

```
## # A tibble: 24 × 3
##    drug  patient_sex heartrate
##    <chr> <chr>           <dbl>
##  1 a     Frank_m            84
##  2 a     Hawkeye_m          64
##  3 a     Henry_m            88
