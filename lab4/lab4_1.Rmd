...
We are only interested in lakeid and scalelength. (select takes out columns - works with variables)
```{r}
...
fish
```
Colon takes out columns from one variable to an end variable: 
```{r}
...
```
The function anyNA says there are no NAs, but I cannot calculate the mean of max. life. 
```{r}
x <- mammals[,11]
colMeans(x)
```
RStudio interprets -999 as a value, as opposed to an NA which is what was intened by the data collectors. 
...
```
```{r}
select(mammals_new, "species", "mass", "genus")
```
