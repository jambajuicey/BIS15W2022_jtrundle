...
library(janitor)
```
Clean names command automatically cleans up the titles of any capitals or spaces or punctuation
```{r}
...
tabyl(fish, lakeid)
```
#n is the number of data values for each of the lakeids
...
```
```{r}
filter(fish, length %in% c(355, 450))
```
...
```
```{r}
filter(fish, between(length, 200, 230))
```
...
```{r}
filter(fish, near(radii_length_mm, 1.7, tol = 0.3))
```
...
# the & symbol makes it so both conditions are present at once
```{r}
filter(fish, lakeid == "AL" & length > 350)
```
Above and below functions are the same!

```{r}
filter(fish, lakeid == "AL", length > 350)
```
Notice that the `|` operator generates a different result.
# the | symbol makes it so either condition is present, doesn't have to be together
```{r}
filter(fish, lakeid == "AL" | length > 350)
```
