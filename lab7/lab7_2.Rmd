...
There are obviosuly -999 values, and means like gestation time are negative, which doesn't make sense. 

```{r}
life_history %>% 
  purrr::map_df(~sum(is.na(.)))
```
NOt giving us NAs as -999 values, only empty or cells with "NA". 
...
life_history %>% 
  purrr::map_df(~sum(is.na(.)))
```
Now we have more NAs represented as NAs. 
...
```
Lots of lifespans that are 0, so this doesn't make a lot of sense. 
...
naniar::miss_var_summary(life_history)
```
Now we can see that there are NAs in the max_life column 
