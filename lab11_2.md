...
```r
ecosphere <- read_csv("lab11/data/ecs21351-sup-0003-SupplementS1.csv", skip=2, col_names = T) %>% #skip=2 skips the first two rows because it has a header and R doesn't know what to do with it
  clean_names() %>% 
  separate(scientific_name, into = c("genus", "species"), sep=" ") %>% 
  na.omit() #powerful and dangerous 
```
...
Looking at mass of birds with different diets: 
...
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
  facet_grid(.~migratory_strategy)+ #saying to put migratory strategy into columns
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
  labs(title = "Mass by Diet and Migratory Strategy",
       x = NULL,
       y = "Log 10 Mass",
...
Facet_wrap is for one categorical variable, facet_grid can do two categorical variables at once. 
...
```r
ecosphere %>% 
  ggplot(aes(x=mean_eggs_per_clutch, fill=habitat))+
  geom_density(alpha = 0.5)+
  facet_wrap(~habitat)
```
...
ecosphere %>% 
  ggplot(aes(x=mean_eggs_per_clutch, fill=habitat))+
  geom_density()+
  facet_grid(mean_eggs_per_clutch~habitat)+
  theme(axis.text.x = element_text(angle = 60, hjust = 1))
```
