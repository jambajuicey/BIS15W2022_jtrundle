...
```{r}
penguins %>% 
  group_by(island) %>% #groups islands by each island
  summarize(number_NAs=sum(is.na(body_mass_g))) #counts number of NAs
```

Well, that won't work so let's remove the NAs and recalculate.
```{r}
penguins %>% 
  filter(!is.na(body_mass_g)) %>% #takes out NAs from body mass
  group_by(island) %>% 
  summarize(mean_body_mass_g=mean(body_mass_g),
...
```{r}
penguins %>% 
  count(island) #count is a shorthand way of using group by and then summarize 
#use group by and summarize for means, maxs, mins, but for counts, count is shorter
```
```{r}
penguins %>% 
  count(species)
...
penguins %>% 
  count(species)
```
^Shows that there are 152 Adelie penguins in data, and we have 152 total rows in our table restricted to Adlie penguins. 
...
```{r}
penguins %>%
  summarize(across(contains("mm"), mean, na.rm=T)) #across is helpful when you have lots of variables, does counts for all variables at once 
```
