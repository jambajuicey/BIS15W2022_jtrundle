...
msleep %>% 
  ...
  summarize(mean_bodywt_pm= mean(bodywt),
            min_bodywt_pm=min(bodywt),
            max_bodywt_pm=max(bodywt),
            total = n())
```
```{r}
dim(msleep)
...
It probably means that there are some NAs in the data so they cannot calculate the mean. 
...
msleep %>% 
  ...
  select(order, genus, brainwt)
```
We don't have any  data for Cetacea for brain weight. 
