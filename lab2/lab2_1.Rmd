...
```
```{r}
julies_experiment <- sum(person_1, person_2)
julies_experiment
```
...
We cannot calculate the mean when we have missing data, so we need to find a way to deal with them so we can calculate a mean. 
...
```
The function na.rm+T allows us to calculate the mean by removing NAs. 
