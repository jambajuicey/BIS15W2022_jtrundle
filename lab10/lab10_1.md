...
life_history %>% 
  ggplot(aes(x=order)) + 
  geom_bar()+ #use when doing counts for a specific variable
  coord_flip()
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+ #reorder() orders bars 
  geom_col()+
  coord_flip()+
  scale_y_log10()
```
...
elephants %>% 
  ...
  geom_point() + 
  geom_smooth(method=lm, se=F)+
  labs(title="Elephant Age vs. Height", #labs adds titles for plot and axes
       x="Age (years)",
       y="Height (cm)")
```
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x = "Age (years)",
       y = "Height (cm)") +
  theme(plot.title = element_text(size = 14, face = "bold"), #adjusting font size and boldness
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs. Height",
       x = "Age",
       y = "Height") +
  theme(plot.title = element_text(size = rel(1.6), hjust = 0.5)) #rel = relative size, title and axes are scaled relative to one another, hjust = placement of title
```
...
```r
elephants %>% 
  ggplot(aes(x=sex, fill=sex))+ #fill fills with color by sex
  geom_bar()+
...
```r
life_history %>% 
  ggplot(aes(x=gestation, y=log10(mass), size=mass))+ #size changes size of points by mass
  geom_point(na.rm=T)
```
...
homerange %>% 
  ...
  geom_point(size=1.2, color="blue", alpha=0.5)+ #alpha makes dots transparent
  labs(title = "Mass vs. Homerange",
       x="mass",
       y="homerange")
```

![](lab10_1_files/figure-html/unnamed-chunk-32-1.png)<!-- -->
