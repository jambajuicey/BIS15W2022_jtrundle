...
```{r}
p <- homerange %>% #making plot an object
...
```{r}
p+geom_point(aes(shape=thermoregulation, color=thermoregulation), size=1.75) #shape and color based on thermoregulation, easy to include in this way because it's categorial and there are only 2 options
```
...
```{r}
homerange %>% 
  ggplot(aes(x = taxon, fill = trophic.guild)) + #using fill with another categorical variable
  geom_bar() +
  coord_flip() +
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
...
homerange %>% 
  ...
  geom_bar(position = "dodge") + #puts bars of different colors side by side
  coord_flip() +
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill = "Trophic Guild")
...
homerange %>% 
  ...
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + #angle tilts labels
  labs(title = "Observations by Taxon in Homerange Data",
       x = "Taxonomic Group",
       fill= "Trophic Guild")
```
